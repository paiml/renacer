...
macro_rules! contract_pre_extract_heads {
    ...
    ($input:expr) => {{
        let q = &$input;
        debug_assert!(q.len() > 0, "Contract extract_heads: precondition violated — q.len() > 0");
    }};
}
...
// Total: 646 preconditions, 15 postconditions from 277 contracts
